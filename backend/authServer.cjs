...
app.delete('/logout', async (req, res) => {
    ...
    try{
        ...
        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, payload) => {
            ...
            models.users.findOne({email:payload.email}).exec().then((user) => {
                if(user===null){
...
app.post('/login', async (req, res) => {
    ...
    try{
        ...
        await models.users.findOne({email:email}).exec().then((user) => {
            console.log(user)
            if(user===null) {
                logError("NoUserFound");
    ...
    try{
        ...
        sendVerifEmail(email, verifToken).then(mailRes=>{
            ...
            switch(mailRes.status){
                ...
                case 535:
                    return res.status(500).json({code:'InternalError', message: "An issue occured while trying to send the verification email"});
            }
...
app.get('/verifEmail', async (req, res)=>{
    ...
    try{
        ...
        await models.users.findOne({email:b64ToAscii(email)}).exec().then((user) => {
            if(user===null) {
                logError('NoUserFound');
...
async function sendVerifEmail(dest, token) {
    ...
    return new Promise((resolve,reject)=>{
        const mailOptions = {
            ...
            text: process.env.VERIF_MAIL_TEXT.format(dest, process.env.VERIF_URL.format(process.env.AUTH_URL, asciiToB64(dest), token))
        }
        let mailRes = {};
...
async function sendForgottenPasswordEmail(dest, token) {
    return new Promise((resolve,reject)=>{
        const mailOptions = {
            ...
            text: process.env.PWD_MAIL_TEXT.format(process.env.PWD_URL.format(process.env.AUTH_URL, asciiToB64(dest), token))
        }
        let mailRes = {};
        mailTransporter.sendMail(mailOptions, function(err, info){
            if (err) {
