...
app.post("/changePassword", async (req, res) => {
  ...
  if (!resetToken)
    return res.status(400).json({
      code: "MalformedLink",
      message: "The given link is missing the reset token",
    });
  if (!email) {
...
async function sendVerifEmail(dest, token) {
  return new Promise((resolve, reject) => {
    const mailOptions = {
      ...
      text: process.env.VERIF_MAIL_TEXT.format(
        dest,
        process.env.VERIF_URL.format(
...
async function sendForgottenPasswordEmail(dest, token) {
  return new Promise((resolve, reject) => {
    const mailOptions = {
      ...
      text: process.env.PWD_MAIL_TEXT.format(
        process.env.PWD_URL.format(
          process.env.AUTH_URL,
          asciiToB64(dest),
          token,
        ),
...
function logError(msg, ret) {
  ret
    ? console.log("ERROR " + apiCall + " : " + msg, ret)
    : console.log("ERROR " + apiCall + " : " + msg);
}
